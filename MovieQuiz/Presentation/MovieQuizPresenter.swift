...
final class MovieQuizPresenter {
    ...
    private func proceedToNextQuestionOrResults() {
        if isLastQuestion() {
            ...
            resultAlert?.showResult(viewcontroller: viewController as? UIViewController ??  UIViewController(), model: viewModel)
        } else {
...
extension MovieQuizPresenter: QuestionFactoryDelegate {
    ...
    func didLoadDataFromServer() {
        viewController?.hideLoadingIndicator()
        if (questionFactory?.movies.isEmpty ?? true) {
            let viewModel = AlertModel(title: "Ошибка загрузки",
